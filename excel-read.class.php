...
public function getWorksheetList(){
	...
	if($this->excel instanceof Spreadsheet_Excel_Reader){
		${0} = $this->excel->boundsheets;
		foreach(${0} as $id => $val){
			$List[$val['name']] = $id;
		}
	}elseif($this->excel instanceof SimpleXLSX){
...
class SimpleXLSX {
	...
	function worksheet( $worksheet_id ) {
		if ( isset( $this->worksheets[ $worksheet_id ] ) ) {
			...
			trigger_error('Worksheet '.$worksheet_id.' not found.', E_USER_WARNING);
			return false;
		}
	}
	/**
	...
	* @RETURN FALSE | INT
	*/
	function getWorksheetIDbyName($name){
	...
	function _unzip( $filename ) {
		...
		if(!isset($aP['CL'])){
			throw new Exception("{$filename} is not a vaid file");
		}		
			...
			if ($rel["Type"] == SimpleXLSX::SCHEMA_OFFICEDOCUMENT) {
				...
				foreach ($workbookRelations->Relationship as $workbookRelation) {
					if ($workbookRelation["Type"] == SimpleXLSX::SCHEMA_WORKSHEETRELATION) {
						$this->worksheets[ str_replace( 'rId', '', (string) $workbookRelation["Id"]) ] =
